...
class DATASET(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 2. 读取 Mask 和 SDF (H, W, 2)
        mask_sdf_data = np.load(self.masks_path[index])

        # 分离 Mask 和 SDF
        mask = mask_sdf_data[..., 0]  # Channel 0
        sdf_gt = mask_sdf_data[..., 1]  # Channel 1
        ...
        if self.transform is not None:
            ...
            augmentations = self.transform(image=image, masks=[mask,sdf_gt,background])
...
def train(model, loader, optimizer, loss_fn, device, consistency_loss_fn=None, consistency_weight=1.0):
    ...
    for i, ((x), (y1, y2,sdf_gt)) in enumerate(tqdm(loader, desc="Training", total=len(loader))):
        ...
        weight_sdf = 1.0 - uncertrainty_map
        weight_soft = uncertrainty_map
