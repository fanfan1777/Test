...
class SDFHead(nn.Module):
    def __init__(self, in_c1, in_c2, in_c3, in_c4, out_c=1):
        super().__init__()
        # 结构与分割头类似，提取多尺度特征
        self.conv1 = CBR(in_c1, 64, kernel_size=1, padding=0, act=True)
        self.conv2 = CBR(in_c2, 64, kernel_size=1, padding=0, act=True)
        ...
        # 融合层
        self.conv_fuse = nn.Conv2d(4 * 64, out_c, kernel_size=1, padding=0, bias=False)
        # 注意：这里移除了 Sigmoid，因为 SDF 是回归任务，值域是 (-inf, +inf)
...
class ConDSegStage1(nn.Module):
    def __init__(self, H=256, W=256):
        ...
        backbone = resnet50()
        self.input_adapt = nn.BatchNorm2d(3)
        ...
        #TODO修改
        self.sdf_head = SDFHead(64,256,512,1024,1)
    ...
    def forward(self, image):
        ...
        #TODO 修改
        pred_sdf = self.sdf_head(x1,x2,x3,x4)

        return pred,pred_sdf
