...
#### TN3K support
- Keep the official layout under `../data/TN3K/`:
  ```text
  TN3K
  ├── trainval-image/
  ├── trainval-mask/
  ...
  └── test-mask/
  ```
- You no longer need to generate `train.txt` / `val.txt`. The new loader in `utils/data_io.py` deterministically splits the `trainval-*` pairs (default 10% for validation, seeded by the training script's `seed`).
- Adjust the `val_ratio` variable inside `train_stage1.py` and `train.py` if you prefer a different split.
- `test.py` automatically consumes `test-image`/`test-mask` when they exist, ensuring evaluation is run on the held-out TN3K test set.
