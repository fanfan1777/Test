...
def compute_uncertainty(prob_map):
    ...
    """
    eps = 1e-8
    prob_map = torch.clamp(prob_map, eps, 1.0 - eps)
    entropy = -prob_map * torch.log(prob_map) - (1 - prob_map) * torch.log(1 - prob_map)

    # 归一化到 [0, 1]。最大熵在 p=0.5 时，值为 ln(2) ≈ 0.693
    uncertainty = entropy / 0.69314718056

    # 确保范围
    uncertainty = torch.clamp(uncertainty, 0.0, 1.0)
    return uncertainty.detach()  # 这一点很重要：我们不希望通过不确定性计算来反向传播更新概率本身
...
def _resize_with_padding(img, target_size, is_mask=False,pad_value=0):
    ...
    if img.ndim ==2:
        ...
        border_value = (pad_value, pad_value, pad_value, pad_value)
...
class DATASET(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 2. 读取 Mask 和 SDF
        mask_sdf_data = np.load(self.masks_path[index])
        mask = mask_sdf_data[..., 0]  # 0.0 或 1.0
        sdf_gt = mask_sdf_data[..., 1]  # -1.0 到 1.0

        # 3. 构造背景
        background = 1.0 - mask

        # -------------------------------------------------------------
        # [新增步骤 A]: 创建“有效区域掩码” (Valid Region Mask)
        # 初始状态全为 1 (float32)，大小与原图一致
        # -------------------------------------------------------------
        h, w = mask.shape
        valid_region = np.ones((h, w), dtype=np.float32)

        """ Applying Data Augmentation """
        if self.transform is not None:
            # [关键]: 把 valid_region 也放进 masks 列表里一起变换！
            # 这样如果图像旋转了，valid_region 也会跟着旋转，边缘出现的黑边会自动变成 0
            augmentations = self.transform(
                image=image,
                masks=[mask, sdf_gt, background, valid_region]  # <-- 加入 valid_region
            )
            image = augmentations["image"]
            mask = augmentations["masks"][0]
            sdf_gt = augmentations["masks"][1]
            background = augmentations["masks"][2]
            valid_region = augmentations["masks"][3]  # <-- 取回变换后的 valid_region

        """ Resize """
        # Image
        image = _resize_with_padding(image, self.size, pad_value=0)
        image = np.transpose(image, (2, 0, 1))

        # Mask
        mask = _resize_with_padding(mask, self.size, is_mask=True, pad_value=0)
        mask = np.expand_dims(mask, axis=0)

        # SDF (填充值可以随便填了，反正会被忽略，建议填 -1.0 保持逻辑一致)
        sdf_gt = _resize_with_padding(sdf_gt, self.size, is_mask=False, pad_value=-1.0)
        ...
        # -------------------------------------------------------------
        # [新增步骤 B]: 对 valid_region 进行同样的 Resize + Padding
        # 注意：pad_value 必须是 0 (代表无效区域)
        # 结果：中间图像区域是 1，周围 Padding 黑边是 0
        # -------------------------------------------------------------
        valid_region = _resize_with_padding(valid_region, self.size, is_mask=True, pad_value=0)
        valid_region = np.expand_dims(valid_region, axis=0)

        # 返回时带上 valid_region
        return image, (mask, background, sdf_gt, valid_region)
...
def train(model, loader, optimizer, loss_fn, device, consistency_loss_fn=None, consistency_weight=1.0):
    ...
    if consistency_loss_fn is None:
        consistency_loss_fn = UltraSoundConsistencyLoss().to(device)
    else:
        print("UltraSoundConsistencyLoss is not imported")
    ...
    augmentations = nn.Sequential(
        ...
        K.RandomErasing(scale=(0.02, 0.08), ratio=(0.3, 3.3), value=0.0, p=0.5),
        K.RandomGamma(gamma=(0.4, 1.5), gain=(0.8, 1.2), p=0.5),
        K.RandomPlasmaShadow(roughness=(0.1, 0.5), shade_intensity=(-0.6, 0.0), p=0.4)
    )

    for i, ((x), (y1, y2,sdf_gt,valid_region)) in enumerate(tqdm(loader, desc="Training", total=len(loader))):
        ...
        valid_region = valid_region.to(device, dtype=torch.float32)
        optimizer.zero_grad()

        #TODO 修改
        prob_pred, sdf_pred = model(x)

        uncertainty_map = compute_uncertainty(prob_pred)
        weight_sdf = (1.0 - uncertainty_map) * valid_region
        weight_soft = uncertainty_map * valid_region
        ...
        # 计算差的平方
        diff_sq = (sdf_pred - sdf_gt) ** 2
        # 只保留有效区域的误差
        diff_sq_masked = diff_sq * valid_region
        loss_sdf_sup = diff_sq_masked.sum() / (valid_region.sum() + 1e-8)
...
def evaluate(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for i, ((x), (y1, y2,sdf_gt,valid_region)) in enumerate(tqdm(loader, desc="Evaluation", total=len(loader))):
            x = x.to(device, dtype=torch.float32)
            y1 = y1.to(device, dtype=torch.float32)
            y2 = y2.to(device, dtype=torch.float32)
